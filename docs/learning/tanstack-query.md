...
## staleTime vs refetchInterval — 헷갈리기 쉬운 차이

이 둘은 이름이 비슷해서 같은 역할을 한다고 착각하기 쉽다. 하지만 하는 일이 완전히 다르다.

### staleTime — 신선도 판단 기준

`staleTime`은 "이 데이터를 얼마 동안 신선하다고 볼 건지"를 설정한다. 데이터를 받은 직후부터 `staleTime`이 지나면 "오래된 데이터"로 간주한다.

핵심은 **staleTime이 지났다고 자동으로 재요청하지 않는다**는 것이다. "다음 트리거가 왔을 때 재요청해도 된다"는 허가를 줄 뿐이다. 트리거는 컴포넌트 재마운트, 창 포커스 전환, 수동 `refetch()` 같은 것들이다.

```ts
useQuery({
  staleTime: 3 * 60 * 1000, // 3분 동안 신선하다고 봄
  // → 3분이 지나도 자동 재요청 없음
  // → 단지 "다음 트리거 때 재요청해도 됨"으로 바뀌는 것
})
```

### refetchInterval — 주기적 자동 재요청

`refetchInterval`은 "N밀리초마다 자동으로 서버에 재요청해라"는 설정이다. 사용자가 아무 행동을 안 해도 주기적으로 데이터를 갱신한다.

```ts
useQuery({
  refetchInterval: 60 * 1000, // 1분마다 자동 재요청
  // → staleTime과 무관하게 1분마다 실제 요청이 나감
})
```

### 요청 중에 인터벌이 오면?

`refetchInterval`이 60초인데 서버 응답이 느려서 요청이 아직 진행 중일 때 60초가 되면 어떻게 될까?

새 요청을 보내지 않는다. TanStack Query는 이전 요청이 진행 중(`isFetching = true`)이면 인터벌이 와도 스킵한다. 이전 요청이 끝난 다음에 카운트를 새로 시작한다. 요청이 쌓이거나 겹치지 않는다.
